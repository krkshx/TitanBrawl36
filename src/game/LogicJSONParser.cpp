...
std::unique_ptr<LogicJSONNode> LogicJSONParser::parse(const std::string& text) {
    Cursor cur(text);
...
std::string LogicJSONParser::stringify(const LogicJSONNode& node) {
    return node.toString();
}

void LogicJSONParser::writeString(const std::string& value, std::string& out) {
    out += '"';
...
void LogicJSONString::writeToString(std::string& out) const {
    LogicJSONParser::writeString(value_, out);
}

void LogicJSONNumber::writeToString(std::string& out) const {
    if (!isFloat_) {
...
void LogicJSONObject::writeToString(std::string& out) const {
    out += '{';
    bool first = true;
