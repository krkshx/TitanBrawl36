...
#include "titan/game/player/LogicResourceData.cpp"
#include "titan/sc/Debugger.cpp"
...
inline i32 LogicClientAvatar::getCommodityCount(int slot, const LogicData& data) const {
    if (slot < 0 || slot >= 8) {
...
inline void LogicClientAvatar::setCommodityCount(int slot, const LogicData& data, i32 count) {
    if (slot < 0 || slot >= 8) {
...
inline int LogicClientAvatar::commodityCountChangeHelper(int slot, const LogicData& data,
